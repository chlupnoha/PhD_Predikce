...
class DataTransformerPipeline:
    ...
    def sum_water_agg_admix_add_fibres_microsil(cls, df):
        ...
        """
        for k, v in DataTransformerPipeline.col_to_sum.items():
            df = DataTransformer.sum_columns(df, k, v)
        cols_to_drop = (
            DataTransformerPipeline.water_cols
            + DataTransformerPipeline.fineAgg_cols
            + DataTransformerPipeline.coarseAgg_cols
            + DataTransformerPipeline.fibres_cols
            + DataTransformerPipeline.microsil_cols
        )
        try:
    ...
    def make_log_columns(cls, df, cols):
        ...
        try:
            ...
            print(
                f"{cols} variables were logarithmized and original variables dropped."
            )
        finally:
            return df
    ...
    def make_interactions(cls, df, inter_col, except_cols):
        ...
        try:
            ...
            print(
                f"Interactions (product) of {inter_col} and all variables apart from {except_cols} were made and original variables dropped."
            )
        finally:
            return df
